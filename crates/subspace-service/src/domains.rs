...
pub trait LastDomainBlockReceiptProvider<Block: BlockT> : Send{
    async fn get_execution_receipt(&self)-> Option<ExecutionReceiptFor<Block::Header, Block, Balance>>;
}
...

{
    ...
    pub async fn get_last_confirmed_domain_block_receipt(&self) -> Option<ExecutionReceiptFor<Block::Header, Block, Balance>>
    {
        ...
        loop {
            ...
            'peers: for (peer_id, peer_info) in peers_info.iter() {
                ...
                let request = LastConfirmedBlockRequest { domain_id: self.domain_id };
