...
mod tests {
	...
	fn execution_receipts_should_be_kept_against_best_execution_chain_number() {
		...
		let write_receipt_at = |hash: Hash,
		                        number: BlockNumber,
		                        best_execution_chain_number: BlockNumber,
		...
		// Create PRUNING_DEPTH receipts, best_execution_chain_number is 0, no receipt has ever been included on primary chain.
		let block_hash_list = (1..=PRUNING_DEPTH)
			.map(|block_number| {
				let receipt = create_execution_receipt(block_number);
				let block_hash = Hash::random();
		...
		write_receipt_at(
			block_hash,
			PRUNING_DEPTH + 4,
			PRUNING_DEPTH + 3, // Include all the missing receipts.
			&create_execution_receipt(PRUNING_DEPTH + 4),
		);
		assert!(receipt_at(block_hash_list[0]).is_none());
		// block number [1, 2, 3] mapping should be pruned as well.
		(1..=3).for_each(|pruned| {
